...
例如：HTTP(超文本傳輸 port 80)、SMTP(簡單郵件傳輸 port25)、SNMP、FTP(檔案傳輸 port21)、Telnet(遠端登入 port23)、SIP、
SSH(安全遠端登入 port22)、NFS、RTSP、XMPP、Whois、ENRP、TLS DNS(網域名稱服務 Port53) POP3(郵件接收 Port110) NTP(網路時間 Port123)
https(安全超文本傳輸 port443) 網路芳鄰(port UDP 137.138 TCP 139.445)
```
```
常見攻擊手法：
...
---- TCP(Transmission Control Protocol) ----
        ...
        [在TCP協定上的協定]
            HTTP（Hypertext Transfer Protocol，超文字傳輸協定80）
            HTTPS（Hypertext Transfer Protocol over Secure Socket Layer, or HTTP over SSL，安全超文字傳輸協定443）
            FTP（File Transfer Protocol，檔案傳輸協定21）
            POP3（Post Office Protocol, version 3，郵局協定110）
            SMTP（Simple Mail Transfer Protocol，簡單郵件傳輸協定25）
            TELNET（Teletype over the Network，網路電傳23）
            SSH（Secure Shell，用於替代安全性差的TELNET22）
